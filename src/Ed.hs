...
addr2 :: Command -> EdArgs-> Int
addr2 cmd edArgs = fixAddr edArgs $ takeAddr2 $ fromMaybe 1 $ addr cmd
    where
...
ed' cmd edArgs = case cmdName cmd of
        ...
        'a' ->
                  insert >>= (\x -> inputCmd >>=
                  (`ed'` edArgs {buff = iCmd (buff edArgs) x  $ addr1 cmd edArgs + 1, saved = False}))
        'i' ->
                  insert >>= (\x -> inputCmd >>=
                  (`ed'` edArgs {buff = iCmd (buff edArgs) x $ addr1 cmd edArgs, saved = False}))
        'd' ->
                  inputCmd >>=
                  (`ed'` edArgs {buff = deleteLine (buff edArgs) (addr1 cmd edArgs) (addr2 cmd edArgs), saved = False})
        'l' -> do
...
printBuff cmd edArgs allLines =
        putStr $ unlines $ drop
                ((addr1 cmd edArgs) - 1)
                (reverse (drop (length allLines - ((addr1 cmd edArgs) + (addr2 cmd edArgs) - 1)) $ reverse allLines))
